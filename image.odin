...
image_3d :: proc(img: ^Image, x: f32, y: f32, z: f32) {
    ctx := get_context()
    if ctx.cc != nil {
        if img != nil && img.ok && img.simg_ok {
            image_3d_with_size(img, x, y, z, f32(img.width), f32(img.height))
        }
    }
}

image_with_size:: proc(img: ^Image, x: f32, y: f32, w: f32, h: f32) {
    image_3d_with_size(img, x, y, 0, w, h)
}

image_3d_with_size:: proc(img: ^Image, x: f32, y: f32, z: f32, w: f32, h: f32) {
    ctx := get_context()
    if ctx.cc != nil {
        if img != nil && img.ok && img.simg_ok {
            ...
            // if c.a != 255 {
                ...
                if ctx.cc != nil {
                    ...
                    // sgl.load_pipeline(ctx.cc.pipelines.add)
                }
            // }
            ...
            sgl.v3f_t2f(px0, py0, z, u0f, v0f)
            sgl.v3f_t2f(px1, py0, z, u1f, v0f)
            sgl.v3f_t2f(px1, py1, z, u1f, v1f)
            sgl.v3f_t2f(px0, py1, z, u0f, v1f)
            sgl.end()
